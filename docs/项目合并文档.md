...
3. 任何协议变更必须先更新 `Shared/Protocols/`，再改代码。
   - **Sync 协议已升级为 V3（事件流 + 快照并存）**：映射名称 `Local\\DNFSyncBox.KeyboardState.V3`。
...
2. 通过共享内存检测状态：
   ...
   - Sync：读取 `DNFSyncBox.KeyboardState.V3` 心跳或序列号变化。
3. 状态栏展示：
   - 未注入 / 运行中 / 超时失联 等状态提示。
...
2. **状态展示来源**：
   - Helper：读取 `GameHelperStatus_{pid}` 心跳或状态字段。
   - Sync：读取 `DNFSyncBox.KeyboardState.V3` 的 `LastTick/Seq`。
3. **快捷入口**：
...
| SyncMappingName | Sync 共享内存名称 | `Local\\DNFSyncBox.KeyboardState.V3` |
