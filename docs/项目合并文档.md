...
- **MasterGUI**：`game-master.exe` 同级 `./logs/mastergui_debug.log`（Debug）
- **Injector**：`game-injector.exe` 同级 `./logs/injector_debug.log`（Debug）
- **Payload**：`game-payload.dll` 同级 `./logs/payload_debug.log`（Debug）
- **Helper**：`game-payload.dll` 同级 `./logs/game_helper.jsonl`
- **Sync GUI**：`game-master.exe` 同级 `./logs/latest.log`
