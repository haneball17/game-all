...
11. **Payload 统一隐蔽开关与状态机（2026-02-18）**
    - 新增 `Payload/runtime/payload_runtime.*`，统一维护初始化状态机与隐蔽开关。
    - `payload.ini` 新增 `EnableStealth`（默认 `false`），由 `dllmain.cpp` 加载并下发到模块。
    - `HelperMod/SyncMod` 的隐蔽相关动作统一受总开关控制，默认关闭并输出明确日志状态。
    - `Helper` 默认配置改为 `wipe_pe_header=false`，降低误触发风险。
12. **Payload 启动失败回滚（2026-02-18）**
    - `dllmain.cpp` 新增启动回滚点：当模块启动异常且开启严格模式时，自动回滚已启动模块。
    - 新增配置 `StrictStartupRollback=true`，用于控制“异常即回滚”策略。
    - `DLL_PROCESS_DETACH` 改为按“真实启动状态”停止模块，避免重复/无效 Stop。
13. **Helper 地址/偏移配置驱动与日志降噪（2026-02-27）**
    - `HelperMod.cpp` 中基址/偏移改为“默认值 + 配置覆盖”模式，运行时统一走 `g_*` 变量。
    - 新增 `game_helper.ini` 的 `[address]`、`[offset]` 配置段，作为稳定服参数基线。
    - 新增 `[debug] enable_runtime_address_log` 开关（默认 `false`），仅在排障时输出 `runtime_address/runtime_offset`。
    - 扫描 `Payload/modules` 后确认：除 `HelperMod.cpp` 外暂无同类“绝对地址硬编码”待改造项。
...
3. `config-templates/payload.ini`
   ...
   - 新增隐蔽总开关：`EnableStealth`（默认关闭）
   - 新增严格回滚开关：`StrictStartupRollback`（默认开启）
