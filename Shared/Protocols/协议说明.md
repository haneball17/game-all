...
> V2 在 V1 的基础上增加“动作命令区”，用于 UI/快捷键共享状态并支持一次性切换指令。
...
### 校验规则
- 读取/写入时：`Version == 2` 且 `Size == 44` 才认为有效。
...
### 共享内存名称
- `Local\\DNFSyncBox.KeyboardState.V3`

### 结构体：SharedKeyboardStateV3
- **Version**：`3`
- **EventCapacity**：`4096`
- **EventSize**：`16` 字节
- **Size**：`67376` 字节（`16 + 1824 + 4096*16`）

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| Version | uint32 | 协议版本 |
| Magic | uint32 | 魔数校验（`0x33564E44`，即 `DNV3`） |
| WriteHead | int32 | 事件写指针（单调递增） |
| Capacity | uint32 | 事件缓冲区容量（默认 4096） |
| Snapshot | SharedKeyboardStateV2 | 快照兜底（结构同 V2） |
| EventBuffer | byte[] | 事件环形缓冲区 |

**EventBuffer 单条事件（InputEventV3）**
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| SequenceId | uint32 | 事件序号（WriteHead 写入时赋值） |
| VirtualKey | uint8 | 目标键 VK |
| IsDown | uint8 | 1=按下，0=抬起 |
| Flags | uint8 | 保留（目前为 0） |
| Reserved | uint8 | 对齐占位 |
| Timestamp | int64 | 事件时间戳（Stopwatch/QPC） |

### 校验规则
- 读取时：`Version == 3` 且 `Magic == 0x33564E44`。
- 读取时：映射区大小需 ≥ `67376`。
- 写入时：先写 `EventBuffer`，再 `WriteHead++`（保证可见性）。
