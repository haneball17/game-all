...
GAME_CONTROL_CORE_API void game_control_core_apply_profile(
    uint32_t profile_mode,
    ...
    size_t mapping_len,
    uint32_t mapping_behavior_replace,
    const uint8_t* down_ptr,
    const uint32_t* edge_counter_ptr,
    const uint8_t* toggle_state_ptr,
    size_t len,
    uint8_t* keyboard_state_ptr,
    uint32_t* edge_out_ptr,
